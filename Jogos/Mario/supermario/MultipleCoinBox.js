function multiple_coinbox(i,j)			//Konstruktor einer Vielfach-Münzbox
{
	this.i = i;							//damit ist die Münzbox eindeutig identifizierbar
	this.j = j;
	this.x = 32*i;						//world-Koordinaten -> für Animation in GameLoop()
	this.y = (14-j)*32;
	this.activated = false;				//wurde die Box schon ausgelöst?
	
	$('#world').append('<div id=\'multiple_coinbox' + i.toString() + '_' + j.toString() + '\'></div>');		//erzeuge div für die Münzbox und setze sie in die Spielwelt ein
	this.body = $('#multiple_coinbox'+i.toString()+'_'+j.toString());										//hiermit lässt sich das Münzbox-div ansprechen
	...
	$('#world').append('<div id=\'multiple_coinboxcoin' + i.toString() + '_' + j.toString() + '\'></div>');	//erzeuge div für die Münze hinter der Münzbox und setze sie in die Spielwelt ein
	this.coin = $('#multiple_coinboxcoin'+i.toString()+'_'+j.toString());									//hiermit lässt sich das Münzen-div ansprechen
